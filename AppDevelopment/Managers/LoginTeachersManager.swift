...
class LoginTeachersManager {
    
    // MARK: - Declarations
    static let shared = LoginTeachersManager()
    var teachersDb: OpaquePointer?
    let kDbPath: String = "LoginteachersDB.sqlite"
    var fileUrl: URL?
    ...
    func saveTeacher(_ teacher: Teacher) {
        let insertStatementString = "INSERT INTO teacherLogin(name, surname, email, password) VALUES (?, ?, ?, ?);"
    ...
    private func createTeachersTable() {
        let createTableString = "CREATE TABLE IF NOT EXISTS teacherLogin(name TEXT, surname TEXT, email TEXT, password TEXT);"
    ...
    private func getTeachers() -> [Teacher] {
        let queryStatementString = "SELECT * FROM teacherLogin;"
